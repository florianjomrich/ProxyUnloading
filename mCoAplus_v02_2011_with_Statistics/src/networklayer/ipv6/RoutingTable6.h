...
class INET_API RoutingTable6 : public cSimpleModule, protected INotifiable
{
  protected:
    ...
    struct HomeNetInfo
	{
		...
		//Bruno Sousa 2001/02/28
		simtime_t lastUpd; // To control if it is performing handover after a certain threshold.

		int interfaceId; //Id of interface that is connected to Home.


	};
	friend std::ostream& operator<<(std::ostream& os, const HomeNetInfo& homeNetInfo);
  ...
  public:
    ...
    */
	...
	/**
		 ...
		 */
	void updateHomeNetworkInfo(const IPv6Address& hoa, const IPv6Address& ha, const IPv6Address& prefix, const int prefixLength, const int iID);

	const IPv6Address& getHomeNetHA_adr() {return homeNetInfo.homeAgentAddr;}
	const IPv6Address& getHomeNetHoA_adr() {return homeNetInfo.HoA;}
	const IPv6Address& getHomeNetPrefix() {return homeNetInfo.prefix/*.prefix()*/;}
	const simtime_t& getHomeNetLastUpd() {return homeNetInfo.lastUpd;}
	const int& getHomeNetIntId() {return homeNetInfo.interfaceId;}
};
