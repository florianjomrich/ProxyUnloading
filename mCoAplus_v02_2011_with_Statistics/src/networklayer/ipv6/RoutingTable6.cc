...
*/
    ...
    for (unsigned int i=0; i<prefixList.size(); i++)
    {
        ...
        //start BSousa advRtrAddr 10.03.09
        prefix.advRtrAddr = toBool(getRequiredAttr(node, "advRtrAddr"));
        EV << "RoutingTable6 Setting R-flag to: " << prefix.advRtrAddr << endl;

        if (prefix.advRtrAddr){
              if(!prefix.rtrAddress.tryParse(getRequiredAttr(node, "rtrAddr")))
                     opp_error("element <%s> at %s: wrong IPv6Address syntax %s",
                                       node->getTagName(), node->getSourceLocation(), getRequiredAttr(node, "rtrAddr"));
                   EV << "RoutingTable6 Router Address set to " << prefix.rtrAddress << endl;
        }
        //end BSousa
...
const IPv6Address& RoutingTable6::getHomeAddress()
{
	 ...
	 */
	//MCoA2.0
	if (getHomeNetHoA_adr().isUnspecified()){
		return (getHomeNetHoA_adr() );
	}
	return IPv6Address::UNSPECIFIED_ADDRESS;
}
...
void RoutingTable6::updateHomeNetworkInfo(const IPv6Address& hoa, const IPv6Address& ha, const IPv6Address& prefix, const int prefixLength, const int intID)
{
	EV<< "\n++++++ Updating the Home Network Information \n";
	homeNetInfo.HoA = hoa;
	homeNetInfo.homeAgentAddr = ha;
	homeNetInfo.prefix = prefix;


	//Bruno Sousa
	homeNetInfo.lastUpd = simTime();

	homeNetInfo.interfaceId = intID;
