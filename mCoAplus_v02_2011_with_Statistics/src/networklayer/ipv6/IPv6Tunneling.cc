...
int IPv6Tunneling::createTunnel(TunnelType tunnelType, const IPv6Address& entry, const IPv6Address& exit, const IPv6Address& destTrigger)
{
	...
	else if ( tunnelType == T2RH || tunnelType == HA_OPT )
	{
		...
		// fire notification
		IPv6TunAdr *tunadr = new IPv6TunAdr(entry, exit, destTrigger);
		nb->fireChangeNotification(NF_IPv6_TUNNEL_ADDED, tunadr );
	}
	else
	{
...
bool IPv6Tunneling::destroyTunnel(const IPv6Address& src, const IPv6Address& dest, const IPv6Address& destTrigger)
{
	...
	// fire notification
	IPv6TunAdr *tunadr = new IPv6TunAdr(src, dest, destTrigger);
	nb->fireChangeNotification(NF_IPv6_TUNNEL_DELETED, tunadr );
...
void IPv6Tunneling::decapsulateDatagram(IPv6Datagram* dgram)
{
	...
	//controlInfo->setInterfaceId(-1);
	dgram->setControlInfo(controlInfo);
	//EV << "TUNNEL: got interface id = " << ie->getInterfaceId() << " and in controlinfo = " << controlInfo->getInterfaceId() << endl;

	send(dgram, "linkLayerOut");

	EV << "DBG decapsulating packet from "<< dgram->getSrcAddress() << " to " << dgram->getDestAddress()  <<  endl;
		 ...
		 && (dgram->getTransportProtocol() != IP_PROT_IPv6EXT_MOB) )
	{
		...
		EV << "DBG Checking Route Optimization  from "<< dgram->getSrcAddress() << " to " << dgram->getDestAddress()
				  << "in tunnel from " << controlInfo->getSrcAddr() << " to " << controlInfo->getDestAddr()
				  << " with proto =" << dgram->getTransportProtocol() <<
				  endl;
		...
		// regarding its CoA...
		IPv6Address aux = IPv6Address::UNSPECIFIED_ADDRESS;


		/*
		mipv6->triggerRouteOptimization( dgram->getSrcAddress(),
				aux,
				MCOA_UNSIGNED_BID,
				ie->ipv6Data()->getMNHomeAddress(), ie);
		*/
		mipv6->triggerRouteOptimization( dgram->getSrcAddress(),
						aux,
						MCOA_UNSIGNED_BID,
						mipv6->getRt6()->getHomeNetHoA_adr(), ie);
	}
...
void IPv6Tunneling::prtTunnels(){

	for (Tunnels::iterator it=tunnels.begin(); it!=tunnels.end(); it++)
	{
		EV << "Tunnel with id  " << it->first << endl << it->second << endl;
	}
}


void IPv6Tunneling::receiveChangeNotification(int category, const cPolymorphic *details)
{
