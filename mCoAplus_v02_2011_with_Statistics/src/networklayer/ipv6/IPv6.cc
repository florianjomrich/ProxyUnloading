...
void IPv6::endService(cPacket *msg)
{
    ...
    else
    {
    	...
    	EV << "Message received on port " << msg->getArrivalGate()->getName() << endl; //lowerTunnelingIn
    	if (msg->getArrivalGate()->isName("lowerTunnelingIn")){
    		EV << "Setting Tunnel to true" << endl;
    		//curIE = getSourceInterfaceFrom(msg);
    		isTunneled = true;
    	}
    	EV << "=== Received on interface " << getSourceInterfaceFrom(msg, isTunneled)->getInterfaceId() << endl;
...
InterfaceEntry *IPv6::getSourceInterfaceFrom(cPacket *msg, bool isTunneled)
{
    ...
    if (!isTunneled){
	   ...
	   // EV << "Interface ID with getId= " << ift->getInterfaceByNetworkLayerGateIndex(g->getId())->getInterfaceId() << endl;
		...
		aux= g ? ift->getInterfaceByNetworkLayerGateIndex(g->getIndex()) : NULL;
    }else{
    	//aux = curIE;
		...
		if (dynamic_cast<IPv6ControlInfo*>(crtrl) != NULL){
			IPv6ControlInfo *controlInfo = check_and_cast<IPv6ControlInfo*>(crtrl);
			EV << "IPv6 Control Tunneled = " << controlInfo->getInterfaceId() << endl;
...
*/
    ...
    if (destIE!=NULL)
    {
    	...
    	if ( nextHop != IPv6Address::UNSPECIFIED_ADDRESS ){
    		...
    		EV << "FOUND netxHop " << nextHop << " with Mac Address " << mac_to_use <<  endl;
		...
		if (interfaceId != destIE->getInterfaceId() && interfaceId != -1){
			EV << "IN IPV6 LAYER IT WAS CHOSEN A DIFFERENT INTERFACE!!!!!!!!!!" << interfaceId << " , " << destIE->getInterfaceId() << endl;
			EV << "Setting Correct MAC ADDRESS FROM  " << mac_to_use ;
			mac_to_use = nd->resolveNeighbour_MCoA(nextHop, destIE->getInterfaceId());
...
void IPv6::routePacket(IPv6Datagram *datagram, InterfaceEntry *destIE, bool fromHL, bool isTunneled)
{
...
cPacket *IPv6::decapsulate(IPv6Datagram *datagram, bool isTunneled)
{
    ...
    if (isTunneled){ // Free memory on tunnels not previously freed
		...
		if (dynamic_cast<IPv6ControlInfo*>(crtrl) != NULL){
			IPv6ControlInfo *controlInfo = check_and_cast<IPv6ControlInfo*>(crtrl);
			EV << "IPv6 Control Tunnel delete = " << controlInfo->getInterfaceId() << endl;
		}
		delete crtrl;
	}
...
void IPv6::parse_ipv6_datagram_for_mcoa(IPv6Datagram* datagram, InterfaceEntry* ie, bool rcv) {
	...
	if ( rt->isMobileNode() )
	 {
		...
		if (mipv6){
			...
			if (proto == IP_PROT_TCP || proto == IP_PROT_UDP || proto== IP_PROT_SCTP  ){

				if (!rcv){
					if (datagram->getDestAddress() != ie->ipv6Data()->getHomeAgentAddress() ){
							EV << "MCOA - UPDATING LIST OF CNs on sending CN= " << datagram->getDestAddress() << " src=" << datagram->getSrcAddress()  << endl;
							mipv6->updateCnList(datagram->getDestAddress(), mipv6->get_and_calcBID(datagram->getSrcAddress(), ie ) ) ;
					}
				}else {
					if (!datagram->getSrcAddress().isUnspecified() && !datagram->getDestAddress().isUnspecified()){
						EV << "MCOA - UPDATING LIST OF CNs on receiving CN= " << datagram->getSrcAddress()  << " src=" <<  datagram->getDestAddress()  << endl;
						mipv6->updateCnList(datagram->getSrcAddress(), mipv6->get_and_calcBID(datagram->getDestAddress(), ie ) ) ;
					}
				}
			}else {
				EV << "Ignoring update of CN list for packet with protocol" << proto << endl;
			}
		}
	 }
}
...
void IPv6::processCommand(const cXMLElement& node) {
	...
	if (!strcmp(node.getTagName(), "new-IPv6Status"))
	{
		...
		//EV << "BRUNO DBG - processCmd setting frameCapacity to " << frameCapacity << endl;
		 EV << "BRUNO DBG - processCmd setting isActive to " << this->isActive  << endl;
