...
IPv6InterfaceData::IPv6InterfaceData()
{
	...
	int mipv6_ha_bindinglifetime, mipv6_rr_bindinglifetime, ipv6_reachable_time ;
	int ipv6_retrans_timer, ipv6_delay_first_probe;
	int ipv6_adv_reachable_time,ipv6_adv_retrans_timer;

	mipv6_ha_bindinglifetime = 	rt6->par("max_ha_binding_lifetime");
	mipv6_rr_bindinglifetime = 	rt6->par("max_rr_binding_lifetime");

	ipv6_reachable_time = 		rt6->par("ipv6_reachable_time");
	ipv6_retrans_timer = 		rt6->par("ipv6_retrans_timer");
	ipv6_delay_first_probe = 	rt6->par("ipv6_delay_first_probe");
	ipv6_adv_reachable_time = 	rt6->par("ipv6_adv_reachable_time");
	ipv6_adv_retrans_timer  = 	rt6->par("ipv6_adv_retrans_timer");

	if (mipv6_ha_bindinglifetime == 0)
		mipv6_ha_bindinglifetime = MIPv6_MAX_HA_BINDING_LIFETIME;

	if (mipv6_rr_bindinglifetime == 0)
		mipv6_rr_bindinglifetime = MIPv6_MAX_RR_BINDING_LIFETIME;

	EV << "MAX BINDING LIFETIMES HA = " << mipv6_ha_bindinglifetime << " and RR = " << mipv6_rr_bindinglifetime << endl;
    ...
    hostConstants.maxRRBindingLifeTime = mipv6_rr_bindinglifetime; // Bruno Sousa
    hostConstants.maxHABindingLifeTime = mipv6_ha_bindinglifetime; // Bruno Sousa
    ...
    nodeConstants.reachableTime = ipv6_reachable_time; //Bruno Sousa
    //nodeConstants.retransTimer = IPv6_RETRANS_TIMER;
    nodeConstants.retransTimer = ipv6_retrans_timer; //Bruno Sousa
    //nodeConstants.delayFirstProbeTime = IPv6_DELAY_FIRST_PROBE_TIME;
    nodeConstants.delayFirstProbeTime = ipv6_delay_first_probe; //Bruno Sousa
    ...
    hostVars.retransTimer = ipv6_retrans_timer; //Bruno Sousa
    ...
    rtrVars.advReachableTime = ipv6_adv_reachable_time; //Bruno SOusa
    rtrVars.advRetransTimer = ipv6_adv_retrans_timer; //Bruno SOusa
    rtrVars.advCurHopLimit = IPv6_DEFAULT_ADVCURHOPLIMIT;
    rtrVars.advDefaultLifetime = 3*rtrVars.maxRtrAdvInterval;

    //Bruno Sousa
    if ( rt6->isMobileNode() ){
    	homeInfo.HoA = IPv6Address::UNSPECIFIED_ADDRESS;
    	homeInfo.homeAgentAddr = IPv6Address::UNSPECIFIED_ADDRESS;
    	homeInfo.prefix = IPv6Address::UNSPECIFIED_ADDRESS;
    }
#if USE_MOBILITY
...
//Bruno Sousa
void IPv6InterfaceData::choosePreferredAddress2()
{
    // do we have addresses?
    if (addresses.size()==0)
    {
    ...
    //Only choses the Hoa address as prefered Address
    for (AddressDataVector::const_iterator it=addresses.begin(); it!=addresses.end(); it++){
    	if(it->addrType == IPv6InterfaceData::HoA){
    		preferredAddr = it->address;
    		break;
    	}
...
/*
 * Method to set the preferred address based on an address that is chosen for binding operation.
 */
void IPv6InterfaceData::setPreferredAddress(const IPv6Address& adr){
	EV << "IPv6InterfaceData setting pref Addr to " << adr << endl;
	preferredAddr = adr;
}
...
//Bruno Sousa 2010/5/10
const IPv6Address& IPv6InterfaceData::getGlobalAddressReverse(AddressType type) const
{
    for (AddressDataVector::const_reverse_iterator it=addresses.rbegin(); it != addresses.rend(); ++it)
        if (it->address.isGlobal() && it->addrType == type )  // FIXME and valid, 25.9.07 - CB
            return it->address;
    return IPv6Address::UNSPECIFIED_ADDRESS;
}
...
void IPv6InterfaceData::assignAddress(const IPv6Address& addr, bool tentative, simtime_t expiryTime, simtime_t prefExpiryTime, bool hFlag)
{
	...
	if (!hasAddress(addr)){
		...
		EV << "[DBG] >>>> Assining IPv6 address " << addr << " tentative ?" << tentative << " with type  " << a.addrType << endl;
	}else {
		EV << "[DBG] >>>> UPDATING IPv6 address " << addr << " tentative ?" << tentative  << endl;
		for (AddressDataVector::iterator it=addresses.begin(); it!=addresses.end(); it++)
		{
			if (it->address == addr)
			{
				...
				{
					if (hFlag == true)
						it->addrType = HoA; //if H-Flag is set then the auto-conf address is the Home address -.....
					else
						it->addrType = CoA; // else it is a care of address (CoA)
				}
			}
		}
	}
...
IPv6Address IPv6InterfaceData::removeAddress(IPv6InterfaceData::AddressType type)
{
	...
	for (AddressDataVector::iterator it=addresses.begin(); it!=addresses.end(); ++it ) // 24.9.07 - CB
	{
		if ( (*it).addrType == type )
		{
			...
			EV << "[DBG] >>> Removing CoA address --> "  << addr << endl;

			addresses.erase(it);


		}else {
			EV << "[DBG] >>>  address not removed --> "  << it->address << endl;
		}
	}
...
bool IPv6InterfaceData::removeAddressInVec(const IPv6Address & inaddr){

	bool res=false;
	for (AddressDataVector::iterator it=addresses.begin(); it!=addresses.end(); ++it ) // 24.9.07 - CB
	{
		if((*it).address == inaddr ){
			if (getAddressType(it->address)==CoA){//Bruno Sousa 2010/7/26
				addresses.erase(it);
				res=true;
				break;
			}
	...
	if (res){
		choosePreferredAddress2();
		EV << "NOW prefered address is: " << preferredAddr ;
	}
	return res;
