...
// FIXME address validity check missing. introduce hasValidAddress(addr, now) which would compare lifetimes too?
    ...
    //Bruno Sousa
    void setPreferredAddress (const IPv6Address& adr);
    void resetPreferredAddress() {choosePreferredAddress();}
    ...
    bool removeAddressInVec(const IPv6Address & address);
...
//############## Additional Function defined by Zarrar YOusaf @ CNI, Uni Dortmund  #########
    ...
    //Reserved Order (Bruno SOusa
    const IPv6Address & getGlobalAddressReverse(AddressType type = HoA) const;
    /**
...
/* Bruno Sousa
     ...
     *
     */
    AddressDataVector getAddresses() const
    {
        return addresses;
    }

    void setAddresses(AddressDataVector addresses)
    {
