...
void IPv6NeighbourDiscovery::initialize(int stage)
{
	...
	WATCH(maxRAInterval);	// (Zarrar 15.07.07)
        ...
        //Bruno Sousa
        HandoverAlgorithm = par("HandoverAlgorithm");
        AlgorithmData1 = par("AlgorithmData1");
        ...
        if (rt6->isRouter())
        	...
        	scheduleAt(simTime()+uniform(0.1,0.5), msg);//Random Host bootup time //Bruno Sousa
        	//scheduleAt(simTime()+uniform(0.4,1), msg);//Random Host bootup time //Bruno Sousa original
...
/*
 *
 * when chosen interface from MIPv6 is different from the one that is selected in the IPv6 layer (neighbour Desicovery mechanisms)
 *
 * TODO:
 * - Check if this does not violate any Neighbour Discovery specification
 * - Not wokring when multiple addresses exist for an interface in the neighbour cache.
 */
const MACAddress& IPv6NeighbourDiscovery::resolveNeighbour_MCoA(const IPv6Address& nextHop, int interfaceId)
{
    Enter_Method("resolveNeighbor_MCoA(%s,if=%d)", nextHop.str().c_str(), interfaceId);
    ...
    Neighbour *nce)
{
    ...
    EV << "Initiating Neighbour Unreachability Detection for " << nceKey->address << " in " << nceKey->interfaceID << endl;;
    ...
    EV << "NUD for simtime at" << simTime()+ie->ipv6Data()->_getDelayFirstProbeTime() << endl;
...
void IPv6NeighbourDiscovery::processNUDTimeout(cMessage *timeoutMsg)
{
    ...
    if ( nceKey == NULL ) // 19.09.07 - CB
    	opp_error("nce->MAC=%s, isRouter=%d", nce->macAddress.str().c_str(), nce->isRouter);
    InterfaceEntry *ie = ift->getInterfaceById(nceKey->interfaceID);
    ...
    if (nce->numProbesSent == (int)ie->ipv6Data()->_getMaxUnicastSolicit())
    {
        ...
        EV << "Remove neighbour " << nceKey->address << " and " << nceKey->interfaceID << endl;
    ...
    for(IPv6NeighbourCache::iterator it=neighbourCache.begin(); it != neighbourCache.end(); it++)
    {
        ...
        if (nce.isDefaultRouter)
        {
            if (simTime()>nce.routerExpiryTime)
            {
                ...
                EV << "Remove neighbour " << key.address << " and " <<key.interfaceID << endl;
    ...
    int interfaceID)
{
	...
	EV << "Remove neighbour " << addr << " and " << interfaceID << endl;
...
{
    Enter_Method_Silent();
    EV<<"----------INITIATING DUPLICATE ADDRESS DISCOVERY----------for address->" << tentativeAddr  <<endl;
...
void IPv6NeighbourDiscovery::processDADTimeout(cMessage *msg)
{
    ...
    //EV << "numOfDADMessagesSent is: " << dadEntry->numNSSent << "for Adr " << dadEntry->address   << endl;
    ...
    else
    {
        ...
        //Bruno Sousa
        KeyMCoADAD keyDAD;// = new KeyMCoADAD(tentativeAddr, ie);
        ...
        if ( it != dadGlobalList.end() )
        {
        	DADGlobalEntry& entry = it->second;
        	 EV << "[DBG] Assigning adr: " << entry.addr  << endl;
        	...
        	for (int i=0; i < ie->ipv6Data()->getNumAddresses(); i++ )
			{
				...
				/*
				 * SET a BID for CoAs after DAD.
				 */
				if (addr.isGlobal()){ // MCoA2.0
					EV << "[DBG] >>>> Assigned Address " << addr << " and checking BID "  << endl;

					mipv6->get_and_calcBID(addr, ie);
				}
			}

        	// if we have MIPv6 protocols on this node we will eventually have to
        	// call some appropriate methods
        	triggerMIPv6(tentativeAddr, entry.hFlag, ie);

        	dadGlobalList.erase(it->first);
        }else {
        	EV << "DBGND >>>ADRr in DAD Global List not found! " << tentativeAddr << " , " << ie->getInterfaceId()  << endl;
        }
...
if (rt6->isRouter() && !(ie->isLoopback()))
	...
	for(int i=0; i< ie->ipv6Data()->getNumAdvPrefixes();i++)
		...
		EV << "DBGND Adding Address " << globalAddress << " with  n Addresses " << ie->ipv6Data()->getNumAddresses()  << endl;
		// TODO: FIXME this works with MyNetConf, but not with FlatnetworkConfigurator
...
void IPv6NeighbourDiscovery::processDADTimeout_MCoA(cMessage *msg)
{
    DADEntry *dadEntry = (DADEntry *)msg->getContextPointer();
    InterfaceEntry *ie = (InterfaceEntry *)ift->getInterfaceById(dadEntry->interfaceId);
    IPv6Address tentativeAddr = dadEntry->address;
    ...
    else
    {
        ...
        EV << "DAD searching for " << tentativeAddr  << " from interface ID " << ie->getInterfaceId() << endl;
        ...
        if ( it != dadGlobalList.end() )
        {
        	...
        	for (int i=0; i < ie->ipv6Data()->getNumAddresses(); i++ )
			{
				 ...
				 * SET a BID for CoAs after DAD.
				 */
				if (addr.isGlobal() && !addr.isUnspecified() && rt6->isMobileNode()){ // MCoA2.0
					EV << "[DBG] >>>> Assigned Address " << addr << " and checking BID "  << endl;
    ...
    else
    {
        ...
        for (int i = 0; i < (int)ra->getPrefixInformationArraySize(); i++)
        {
            ...
            if (prefixInfo.getAutoAddressConfFlag() == true)//If auto addr conf is set
            {
            	...
            	//Have a condition to check the number of interfaces Bruno 2011/02/28
            	//TODO:
            		processRAPrefixInfoForAddrAutoConf_MCoA( prefixInfo, ie, ra->getHomeAgentFlag() ); // then calling the overloaded function for address configuration. The address conf for MN is different from other nodes as it needs to classify the newly formed address as HoA or CoA, depending on the status of the H-Flag. (Zarrar Yousaf 20.07.07)
            }
            ...
            if ( ra->getHomeAgentFlag() && (prefixInfo.getRouterAddress() == true) )//If R-Flag is set and RA is from HA
			{
				...
				//Bruno Sousa
				rt6->updateHomeNetworkInfo(HoA, HA, prefixInfo.getPrefix(), prefixInfo.getPrefixLength(), ie->getInterfaceId()); //populate the HoA of MN, the HA global scope address and the home network prefix
			}
        }
    }
    ...
    IPv6ControlInfo *nsCtrlInfo)
{
    ...
    {
        EV << "Source Address is UNSPECIFIED. Sender is performing DAD " << nsSrcAddr  << " to " << nsDestAddr;
...
void IPv6NeighbourDiscovery::processRAPrefixInfoForAddrAutoConf(IPv6NDPrefixInformation& prefixInfo, InterfaceEntry* ie, bool hFlag)
{
    ...
    for (int i = 0; i < ie->ipv6Data()->getNumAddresses(); i++)
	{
        ...
        if (ie->ipv6Data()->getAddress(i).matches(prefix, prefixLength) == true)
        {
        	...
        	if ( rt6->isMobileNode() && ie->ipv6Data()->getAddressType(i) == IPv6InterfaceData::HoA && ie->ipv6Data()->getNumAddresses() > 2){
        		returnedHome = true;
        		EV <<"[MCOA] MN identifying that it is on the Home network "<<endl;
        	}
        	else
        	{
	        	isPrefixAssignedToInterface = true;
    ...
    /*d) If the prefix advertised does not match the prefix of an address already
	    ...
	    if ( (isPrefixAssignedToInterface == false) && (validLifetime != 0) )
	    {
		    ...
		    //TODO: for now we leave the newly formed address as not tentative,
		    //according to Greg, we have to always perform DAD for a newly formed address.
		    EV << "Assigning new address to: " << ie->getName() << " New addr " << newAddr << endl;
			...
			if ( returnedHome )
			{
				...
				//if (!ie->ipv6Data()->isReturningHome()){
					mipv6->returningHomeMCoA(ie);
			...
			else // non-mobile nodes will never have returnedHome == true, so they will always assign a new address
			{
				...
				{
					//bool isLinkLocalTentative = ie->ipv6Data()->isTentativeAddress( linkLocalAddress );
					bool isLinkLocalTentative = false;

					//Bruno Sousa add addresses.
					ie->ipv6Data()->assignAddress(newAddr, isLinkLocalTentative, simTime()+validLifetime, simTime()+preferredLifetime, hFlag);
					...
					for (int j=0; j < ie->ipv6Data()->getNumAddresses(); j++ )
					{
						...
						// Changed by Bruno Sousa 2010/5/5
						if (ie->ipv6Data()->getAddress(j).getScope() == IPv6Address::LINK){
					...
					//Bruno Sousa
					KeyMCoADAD keyDAD ;
					keyDAD.setAdr(newAddr);
					keyDAD.setIe(ie);
			        // set MIPv6Init structure that will later on be used for initiating MIPv6 protocol after DAD was performed
					dadGlobalList[keyDAD].hFlag = hFlag;
					dadGlobalList[keyDAD].validLifetime = validLifetime;
					dadGlobalList[keyDAD].preferredLifetime = preferredLifetime;
					dadGlobalList[keyDAD].addr = newAddr;
					//dadGlobalList[keyDAD].returnedHome = returnedHome;
					dadGlobalList[keyDAD].CoA = CoA;

					//EV << "DBGND Added to DAD global list " << newAddr << " iID " << ie->getInterfaceId() << endl;
...
void IPv6NeighbourDiscovery::processRAPrefixInfoForAddrAutoConf_MCoA(IPv6NDPrefixInformation& prefixInfo, InterfaceEntry* ie, bool hFlag)
{
	EV << "Processing Prefix Info for address auto-configuration.\n";
    IPv6Address prefix = prefixInfo.getPrefix();
    uint prefixLength = prefixInfo.getPrefixLength();
	    ...
	    if ( (isPrefixAssignedToInterface == false) && (validLifetime != 0) )
	    {
			...
			else // non-mobile nodes will never have returnedHome == true, so they will always assign a new address
			{
				...
				if ( ie->ipv6Data()->getNumAddresses() == 1 )
				{
					...
					// this means DAD is already running or has already been completed
					// create a unicast address with scope > link-local
					isLinkLocalTentative = ie->ipv6Data()->isTentativeAddress( linkLocalAddress );
					// if the link local address is tentative, then we make the global unicast address tentative as well
				}
...
bool IPv6NeighbourDiscovery::isConnectedToWirelessAP(InterfaceEntry *ie)
{
 ...
 bool IPv6NeighbourDiscovery::triggerMIPv6(IPv6Address & adr, bool hFlag, InterfaceEntry *ie){

	 bool res= false;
	 switch(HandoverAlgorithm){

		 /*
		  * Corresponds to standard MIPv6. Based on neighbour cache status.
		  */
		 case 1:
		 default: /* BE CAREFUL WITH DEFAULT */
			 if ( rt6->isMobileNode() )
			 {
				 double res = SIMTIME_DBL(simTime() - rt6->getHomeNetLastUpd());
				EV << " DIF lastupd = " << res << endl;
				Neighbour* nHA = neighbourCache.lookup(rt6->getHomeNetHA_adr(), rt6->getHomeNetIntId());
				EV << "ALGO looking for " << rt6->getHomeNetHA_adr() << " in interface " << rt6->getHomeNetIntId() << endl;

				if (nHA != NULL){
					EV << "ALGO Neighbour Status " << neighbourCache.stateName(nHA->reachabilityState) << endl;
					//EV << "ALGO Neighbour " << nHA << endl;

					/*
					 * According to RFC4861, the following states (STALE; DELAY;PROBE) mean that heighbour is not reachable.
					 */
					if (nHA->reachabilityState == IPv6NeighbourCache::STALE ||
						nHA->reachabilityState == IPv6NeighbourCache::DELAY ||
						nHA->reachabilityState == IPv6NeighbourCache::PROBE){
					//if (res > AlgorithmData1 && hFlag == false ){
						//if ( entry.hFlag == false  ){ // if we are not in the home network, send BUs
							mipv6->initiateMIPv6Protocol(ie, adr);
							res=true;
					}
				}
			 }
